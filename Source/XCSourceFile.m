...
@synthesize embeddedKey = _embeddedKey;
...
//        }];
//    }
    //Modified. From: https://github.com/appsquickly/XcodeEditor/issues/62
    if (_buildFileKey == nil) {
        [[_project objects] enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSDictionary *obj, BOOL *stop) {
            if ([[obj valueForKey:@"isa"] xce_hasBuildFileType]) {
                if ([[obj valueForKey:@"fileRef"] isEqualToString:_key]) {
...
/**
 Adds this file to the project as an `Embedded`, ready to be included in targets.
 This is added function. From: https://github.com/appsquickly/XcodeEditor/issues/62
 */
- (void)becomeEmbedded
{
    if (_type == Framework) {
